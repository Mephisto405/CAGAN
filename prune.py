...
from Util.pruning_util import Generate_Prune_Mask_List, Get_Uniform_RmveList
...
grad_score_list = Get_Content_Aware_Pruning_Score(
    generator=g_ema,
...
print(
    "The content-aware metric scoring takes: "
...
prune_net_mask = Generate_Prune_Mask_List(
    content_aware_pruning_score, net_shape, rmve_list, info_print=args.info_print
)

pruned_generator_dict = Mask_the_Generator(g_ema.state_dict(), prune_net_mask)

pruned_ckpt = {
    "g": pruned_generator_dict,
    "d": model_dict["d"],
    "g_ema": pruned_generator_dict,
}
m_time = datetime.datetime.now().strftime("%Y-%m-%d_%H:%M:%S")
ckpt_file = (
    "./Model/pruned_model/content_aware_pruned_"
    + str(args.remove_ratio)
    + "_"
    + str(args.generated_img_size)
    + "px_model_"
    + m_time
    + ".pth"
)
