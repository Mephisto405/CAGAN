...
import lpips
from Evaluation.image_projection.image_projector import (
    Get_LPIPS_Model_Image,
    Get_PSNR_Model_Image,
    Image_Projector,
    im2tensor,
)
...
# Image Preparation
target_img = (
    Image.open(args.image_file)
    .convert("RGB")
    .resize((args.generated_img_size, args.generated_img_size))
)
...
if args.info_print:
    ...
    print_iters = np.inf
input_img_tensor, output_img_tensor = Image_Projector(
    generator=g_ema,
    device=device,
    per_layer_W=True,
...
lpips_percept = lpips.PerceptualLoss(
    model="net-lin", net="vgg", use_gpu=True, gpu_ids=[int(device[-1])]
)
lpips_percept.eval()
target_img = np.array(target_img)
target_img_tensor = im2tensor(target_img)
lpips_score = Get_LPIPS_Model_Image(
    [output_img_tensor], [target_img_tensor], lpips_percept
)[0][0]
