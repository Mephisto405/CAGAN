...
def Get_Parsing_Net(device):
    ...
    PRETRAINED_FILE = (
        file_path / """./face_parsing/pretrained_model/79999_iter.pth"""
    ).resolve()
...
def Batch_Img_Parsing(img_tensor, parsing_net, device):
    ...
    transformed_tensor = F.interpolate(
        transformed_tensor,
    ...
    for i in range(transformed_tensor.shape[1]):
        transformed_tensor[:, i, ...] = (
            transformed_tensor[:, i, ...] - CHANNEL_MEAN[i]
        ) / CHANNEL_STD[i]
...
def Get_Masked_Tensor(img_tensor, batch_parsing, device, mask_grad=False):
    ...
    mask_float = mask.unsqueeze(0).type(
        torch.FloatTensor
    )  # Make it to a 4D tensor with float for interpolation
    scale_factor = img_tensor.shape[-1] / PARSING_SIZE

    resized_mask = F.interpolate(
        mask_float, scale_factor=scale_factor, mode="bilinear", align_corners=False
    )
...
def Get_Salt_Pepper_Noisy_Image(img_tensor, mask, prob):
    ...
    salt_pepper_noise = (
        np.random.randint(low=0, high=2, size=(img_size, img_size)) * 2 - 1
    )  # s/p noise will be -1 and 1
...
def Get_Weight_Gradient(noisy_img, img_tensor, generator):
    ...
    grad_score_list = [
        (torch.mean(torch.abs(grad), axis=[0, 1, 3, 4])).cpu().numpy()
        for grad in grad_list
    ]
    return grad_score_list


def Get_Content_Aware_Pruning_Score(
    generator, n_sample, batch_size, noise_prob, device, info_print=False
):
    """
    Usage:
    ...
    batch_size_list = [batch_size] * (n_batch - 1) + [
        batch_size + n_sample % batch_size
    ]
    grad_score_list = []
    ...
    for (idx, batch) in enumerate(batch_size_list):
        ...
        for i in range(batch):
            ...
            # Get noisy images
            noisy_img = Get_Salt_Pepper_Noisy_Image(
                single_img, resized_mask, noise_prob
            )
