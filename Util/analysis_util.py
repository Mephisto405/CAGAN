...
from .network_util import Convert_Tensor_To_Image, Get_Layer_Output
...
def Pick_High_Low_Activations(layer_activation, channel_rank):
    ...
    good_channel = np.array(
        [layer_activation[i][channel_rank[-1]] for i in range(num_img)]
    )
    bad_channel = np.array(
        [layer_activation[i][channel_rank[0]] for i in range(num_img)]
    )
...
def Channel_Image_Visualization(
    generated_image, good_channel, bad_channel, value_max, value_min
):
    """
    Usage:
...
def Extract_Training_KD_Loss(exp_dir):
    ...
    for line in open(exp_log_file, "r").readlines():
        if "Iter #" in line:
            index_list = [
                line.find(_str) for _str in [KD_L1_STR, KD_LPIPS_STR, END_STR]
            ]
            kd_l1_loss = float(line[index_list[0] + len(KD_L1_STR) : index_list[1]])
            kd_l1_loss_list.append(kd_l1_loss)

            kd_lpips_loss = float(
                line[index_list[1] + len(KD_LPIPS_STR) : index_list[2]]
            )
