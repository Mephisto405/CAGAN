...
def Get_Network_Shape(model_dict):
    ...
    num_channels = [
        model_dict[key].shape[2] for key in conv_key_list
    ]  # from start to end
...
def Get_Generator_Effective_Weights(generator, noise_z):
    ...
    with torch.no_grad():
        ...
        # Get the latent input first
        if "module" in list(generator.state_dict().keys())[0]:
            style_DP = nn.DataParallel(
                generator.module.style, device_ids=generator.device_ids
            )
        ...
        styled_conv_list = (
            [g_module.conv1] + list(g_module.convs) + [g_module.to_rgbs[-1]]
        )
        for styled_conv in styled_conv_list:
            effective_weight = Get_Styled_Conv_Effective_Weights(
                styled_conv, latent_W, gpu_device_ids
            )
...
def Get_Styled_Conv_Effective_Weights(styled_conv, latent_W, gpu_device_ids=None):
    ...
    if gpu_device_ids is None:
        ...
        modulation_DP = nn.DataParallel(
            styled_conv.conv.modulation, device_ids=gpu_device_ids
        )
    ...
    weight = (
        styled_conv.conv.scale * styled_conv.conv.weight.cpu() * style.cpu()
    )  # Place the memory on cpu
