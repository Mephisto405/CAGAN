...
from .network_util import (
    Get_Generator_Effective_Weights,
    Get_Generator_Styles,
    Get_Layer_Output,
    Get_Network_Shape,
)
from .pruning_metrics import (
    Get_ASV_Score,
    Get_Incoming_L1Norm,
    Get_L1_Normalized_ASV,
    Get_L2_Normalized_ASV,
    Get_Map_L1_Norm,
    Get_Outgoing_L1Norm,
)
...
def Get_Map_Based_Score(generator, noise_z, net_shape, metric, info_print=False):
    ...
    for layer_id in range(num_layer):
        ...
        for i in range(num_batch):
            layer_out = Get_Layer_Output(
                generator, noise_z[i * BATCH_SIZE : (i + 1) * BATCH_SIZE], layer_id
            )
...
def Get_Network_Score_List(generator, noise_z, metric, info_print=True):
    ...
    if metric == "Random":
        ...
        Network_Score_List = Get_Map_Based_Score(
            generator, noise_z, net_shape, metric, info_print
        )

    elif metric in ["l1-in", "l1-out"]:
        Network_Score_List = Get_Weight_Based_Score(
            generator, noise_z, metric, info_print
        )

    elif metric in ["l1-style"]:
        Network_Score_List = Get_Style_Based_Score(
            generator, noise_z, metric, info_print
        )
