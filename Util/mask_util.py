...
def Mask_the_Generator(model_dict, net_mask_list):
    ...
    # Masking operation
    pruned_dict["input.input"] = model_dict["input.input"].cpu()[
        :, net_mask_list[0], ...
    ]
...
def Mask_Styled_Conv_Key(model_dict, pruned_dict, net_mask_list, styled_conv_key):
    ...
    for idx, key in enumerate(styled_conv_key):
        ...
        masked_weight = model_dict[key].cpu()[:, output_mask, ...][
            :, :, input_mask, ...
        ]
...
def Mask_toRGB_Key(model_dict, pruned_dict, net_mask_list, toRGB_key):
    ...
    for idx in range(len(toRGB_key) // NUM_KEY_EACH_LAYER):
        layer_mask = net_mask_list[
            idx * 2 + 1
        ]  # the layer idx corresponding to the toRGB module
        conv_key, mod_weight_key, mod_bias_key = toRGB_key[
            idx * NUM_KEY_EACH_LAYER : (idx + 1) * NUM_KEY_EACH_LAYER
        ]
