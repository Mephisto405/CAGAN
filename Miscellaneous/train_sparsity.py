...
from distributed import get_world_size, reduce_loss_dict, reduce_sum
...
from Util.Calculators import GENERATOR_FLOPS_256PX, Styled_Conv_FLOPCal
from Util.GAN_Slimming_Util import VGGFeature, perceptual_loss
from Util.mask_util import Mask_the_Generator
from Util.network_util import Build_Generator_From_Dict, Get_Network_Shape
from Util.pruning_util import (
    Generate_Prune_Mask_List,
    Get_Network_Score_List,
    Get_Uniform_RmveList,
)
...
parser.add_argument(
    "--size", type=int, default=train_sparsity_hyperparams.generated_img_size
)
parser.add_argument(
    "--channel_multiplier",
    type=int,
    default=train_sparsity_hyperparams.channel_multiplier,
)
...
parser.add_argument(
    "--load_train_state", type=bool, default=train_sparsity_hyperparams.load_train_state
)

parser.add_argument(
    "--iter", type=int, default=train_sparsity_hyperparams.training_iters
)
...
parser.add_argument(
    "--r1", type=float, default=train_sparsity_hyperparams.discriminator_r1
)
parser.add_argument(
    "--path_regularize",
    type=float,
    default=train_sparsity_hyperparams.generator_path_reg_weight,
)
parser.add_argument(
    "--path_batch_shrink",
    type=int,
    default=train_sparsity_hyperparams.path_reg_batch_shrink,
)
parser.add_argument(
    "--d_reg_every", type=int, default=train_sparsity_hyperparams.d_reg_freq
)
parser.add_argument(
    "--g_reg_every", type=int, default=train_sparsity_hyperparams.g_reg_freq
)
parser.add_argument(
    "--mixing", type=float, default=train_sparsity_hyperparams.noise_mixing
)

parser.add_argument(
    "--sparsity_eta", type=float, default=train_sparsity_hyperparams.sparsity_eta
)
parser.add_argument(
    "--init_step", type=float, default=train_sparsity_hyperparams.init_step
)
parser.add_argument(
    "--model_prune_freq",
    type=float,
    default=train_sparsity_hyperparams.model_prune_freq,
)
parser.add_argument(
    "--lay_rmve_ratio", type=float, default=train_sparsity_hyperparams.lay_rmve_ratio
)
parser.add_argument(
    "--num_rmve_channel",
    type=float,
    default=train_sparsity_hyperparams.num_rmve_channel,
)
parser.add_argument(
    "--prune_metric", type=str, default=train_sparsity_hyperparams.prune_metric
)
parser.add_argument(
    "--pruning_mode", type=str, default=train_sparsity_hyperparams.pruning_mode
)

parser.add_argument(
    "--n_sample", type=int, default=train_sparsity_hyperparams.val_sample_num
)
parser.add_argument(
    "--val_sample_freq", type=int, default=train_sparsity_hyperparams.val_sample_freq
)
parser.add_argument(
    "--model_save_freq", type=int, default=train_sparsity_hyperparams.model_save_freq
)
parser.add_argument(
    "--fid_n_sample", type=int, default=train_sparsity_hyperparams.fid_n_sample
)
parser.add_argument(
    "--fid_batch", type=int, default=train_sparsity_hyperparams.fid_batch
)

parser.add_argument(
    "--teacher_ckpt", type=str, default=train_sparsity_hyperparams.teacher
)
parser.add_argument(
    "--kd_l1_lambda", type=float, default=train_sparsity_hyperparams.kd_l1_lambda
)
parser.add_argument(
    "--kd_percept_lambda",
    type=float,
    default=train_sparsity_hyperparams.kd_percept_lambda,
)
parser.add_argument(
    "--kd_l1_mode", type=str, default=train_sparsity_hyperparams.kd_l1_mode
)
parser.add_argument(
    "--kd_percept_mode", type=str, default=train_sparsity_hyperparams.kd_percept_mode
)
...
def Print_Experiment_Status(exp_log_file):
    ...
    experiment_status_str += (
        ...
        + str(args.path)
        + "\n"
        + "    Multi-Layer Perceptron Num Layers: "
        + str(args.n_mlp)
        + "\n"
        ...
        + "\n"
        + "    GPU Device IDs: "
        + str(train_sparsity_hyperparams.gpu_device_ids)
        + "\n"
        + "    Number of GPUs: "
        + str(n_gpu)
        + "\n\n"
        ...
        + str(args.lr)
        + "\n"
        + "    Generator Path Regularization Frequency: "
        + str(args.g_reg_every)
        + "\n"
        + "    Path Regularization Weight: "
        + str(args.path_regularize)
        + "\n"
        + "    Path Batch Shrink Ratio: "
        + str(args.path_batch_shrink)
        + "\n"
        + "    Discriminator Regularization Frequency: "
        + str(args.d_reg_every)
        + "\n"
        + "    Discriminator Regularization Weight: "
        + str(args.r1)
        + "\n"
        + "    Noise Mixing: "
        + str(args.mixing)
        ...
        + "\n"
        + "    Eta: "
        + str(args.sparsity_eta)
        + "\n"
        + "    Init_Step: "
        + str(args.init_step)
        + "\n"
        + "    Pruning Metric: "
        + str(args.prune_metric)
        + "\n"
        + "    Pruning Mode: "
        + str(args.pruning_mode)
        + "\n"
        + "    Global Remove Channel Number: "
        + str(args.num_rmve_channel)
        + "\n"
        + "    Layer Remove Ratio: "
        + str(args.lay_rmve_ratio)
        + "\n"
        + "    Model Prune Freqeuncy: "
        + str(args.model_prune_freq)
        + "\n\n"
        + "  Validation Params: "
        + "\n"
        ...
        + str(args.val_sample_freq)
        + "\n"
        + "    Model Saving Frequency: "
        + str(args.model_save_freq)
        + "\n"
        + "    FID Sample Num: "
        + str(args.fid_n_sample)
        + "\n"
        + "    FID Sample Batch Size: "
        + str(args.fid_batch)
        + "\n\n"
    )
    ...
    if args.teacher_ckpt is not None:
        experiment_status_str += (
            ...
            + "\n"
            + "    L1 Knowledge Distillation Weight: "
            + str(args.kd_l1_lambda)
            + "\n"
            + "    L1 Knowledge Distillation Mode: "
            + str(args.kd_l1_mode)
            + "\n"
            + "    Percept Knowledge Distilation Weight: "
            + str(args.kd_percept_lambda)
            + "\n"
            + "    Percept Knowledge Distilation Mode: "
            + str(args.kd_percept_mode)
            + "\n\n"
        )
...
def KD_loss(args, teacher_g, noise, fake_img, fake_img_list, percept_loss):
    ...
    if args.kd_l1_mode == "Output_Only":
        kd_l1_loss = args.kd_l1_lambda * torch.mean(
            torch.abs(fake_img_teacher - fake_img)
        )
        ...
        loss_list = [
            ...
            for (fake_img_teacher, fake_img) in zip(
                fake_img_teacher_list, fake_img_list
            )
        ]
    ...
    # kd_percept_loss
    if (
        args.size > train_sparsity_hyperparams.PERCEPT_LOSS_IMAGE_SIZE
    ):  # pooled the image for LPIPS for memory saving
        pooled_kernel_size = (
            args.size // train_sparsity_hyperparams.PERCEPT_LOSS_IMAGE_SIZE
        )
        fake_img = F.avg_pool2d(
            fake_img, kernel_size=pooled_kernel_size, stride=pooled_kernel_size
        )
        fake_img_teacher = F.avg_pool2d(
            fake_img_teacher, kernel_size=pooled_kernel_size, stride=pooled_kernel_size
        )

    if args.kd_percept_mode == "LPIPS":
        kd_percept_loss = args.kd_percept_lambda * torch.mean(
            percept_loss(fake_img, fake_img_teacher)
        )
...
def D_Loss_BackProp(
    generator, discriminator, real_img, args, device, loss_dict, d_optim
):
    """
    Usage:
...
def G_Loss_BackProp(
    generator, discriminator, args, device, loss_dict, g_optim, teacher_g, percept_loss
):
    """
    Usage:
    ...
    # GAN Loss
    noise = mixing_noise(args.batch, args.latent, args.mixing, device)
    fake_img_list, style_list = generator(
        noise, return_rgb_list=True, return_style_scalars=True
    )
    ...
    if teacher_g is not None:
        kd_l1_loss, kd_percept_loss = KD_loss(
            args, teacher_g, noise, fake_img, fake_img_list, percept_loss
        )
...
def Get_Network_Prune_Mask(network_score, g_ema, args):
    ...
    if args.pruning_mode == "Layer_Uniform":
        rmve_list = Get_Uniform_RmveList(
            train_sparsity_hyperparams.GENERATOR_SHAPE_256PX, args.lay_rmve_ratio
        )
        net_shape = Get_Network_Shape(g_ema.state_dict())
        prune_net_mask = Generate_Prune_Mask_List(
            network_score, net_shape, rmve_list, info_print=False
        )
...
def Prune_Generator(generator, g_ema, discriminator, args, device):
    ...
    network_score = Get_Network_Score_List(
        g_ema, noise_z, metric=args.prune_metric, info_print=False
    )
    ...
    g_ema_parallel = nn.DataParallel(
        g_ema, device_ids=train_sparsity_hyperparams.gpu_device_ids
    )
    g_module = Build_Generator_From_Dict(pruned_g_dict, size=args.size).to(device)
    generator = nn.DataParallel(
        g_module, device_ids=train_sparsity_hyperparams.gpu_device_ids
    )
    ...
    for iter_idx in range(args.start_iter, args.iter):
        ...
        if iter_idx % 1 == 0:
            exp_log_file.write(
                ...
                + str(iter_idx)
                + " Train Time: "
                + str(round(iter_end_time - iter_start_time, 2))
                + " D_Loss: "
                + str(round(d_loss_val, 3))
                + " G_Loss: "
                + str(round(g_loss_val, 3))
                + " Sparse_Loss: "
                + str(round(sparse_loss_val, 3))
                + " KD_L1_Loss: "
                + str(round(kd_l1_loss_val, 3))
                + " KD_Percept_Loss: "
                + str(round(kd_percept_loss_val, 3))
                + " D_Reg: "
                + str(round(r1_val, 3))
                + " G_Reg: "
                + str(round(path_loss_val, 3))
                + " G_Mean_Path: "
                + str(round(mean_path_length_avg, 4))
                + "\n"
            )
        ...
        if (iter_idx % args.model_save_freq == 0) and (iter_idx > 0):
            g_ema_parallel.eval()
            g_ema_fid = Get_Model_FID_Score(
                generator=g_ema_parallel,
                batch_size=args.fid_batch,
                num_sample=args.fid_n_sample,
                device=device,
                gpu_device_ids=train_sparsity_hyperparams.gpu_device_ids,
                info_print=False,
            )
        ...
        if (iter_idx % args.model_prune_freq == 0) and (iter_idx > 0):
            ...
            (
                ...
                g_optim,
                d_optim,
            ) = Prune_Generator(generator, g_ema, discriminator, args, device)
            ...
            exp_log_file.write(
                ...
                + "\n"
                + "FLOPs %: "
                + str(round(g_ema_FLOPs / GENERATOR_FLOPS_256PX * 100, 2))
                + "\n\n"
            )
...
if __name__ == "__main__":
    ...
    loader = data.DataLoader(
        train_dataset, batch_size=args.batch, shuffle=True, num_workers=8
    )
    ...
    if args.teacher_ckpt is not None:
        teacher = torch.load(
            args.teacher_ckpt, map_location=lambda storage, loc: storage
        )
        teacher_g = Build_Generator_From_Dict(teacher["g_ema"], size=args.size).to(
            device
        )
        teacher_g.eval()
        ...
        if args.kd_percept_mode == "LPIPS":
            percept_loss = lpips.PerceptualLoss(
                ...
                net="vgg",
                use_gpu=True,
                gpu_ids=[train_sparsity_hyperparams.gpu_device_ids[0]],
            )
        elif args.kd_percept_mode == "VGG":
    ...
    if args.distributed:
        generator = nn.DataParallel(
            generator, device_ids=train_sparsity_hyperparams.gpu_device_ids
        )
        discriminator = nn.DataParallel(
            discriminator, device_ids=train_sparsity_hyperparams.gpu_device_ids
        )
        if teacher_g is not None:
            teacher_g = nn.DataParallel(
                teacher_g, device_ids=train_sparsity_hyperparams.gpu_device_ids
            )
    ...
    exp_log_file.write(
        "\n"
        + "Total training time: "
        + str(round(train_end_time - train_start_time, 3))
    )
    exp_log_file.close()
