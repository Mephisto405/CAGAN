...
def Image_Projector(
    ...
    if isinstance(target_im, list):  # Image Batch
        ...
        target_im_tensor = target_im_tensor.reshape(
            (1,) + tuple(target_im_tensor.shape)
        ).to(
            device
        )  # transform the tensor from 3D to 4D

    # Average W
    avg_W = Get_Avg_W_as_Latent(generator, device, per_layer_W, is_generator_DPmodule)
    avg_W = torch.repeat_interleave(
        avg_W, target_im_tensor.shape[0], dim=0
    )  # Copy Avg_W to have the same batch size as image batch
