...
def G_Loss_BackProp(
    generator, discriminator, args, device, loss_dict, g_optim, teacher_g
):
    """
    Usage:
    ...
    if teacher_g is not None:
        if args.kd_mode == "Output_Only":
            ...
            kd_l1_loss = args.kd_lambda * torch.mean(
...
def train(
    args, loader, generator, discriminator, g_optim, d_optim, g_ema, device, teacher_g
):
    ...
    for iter_idx in range(args.start_iter, args.iter):
        ...
        (
            time_train_D_g_forward,
            time_train_D_d_forward,
            time_train_D_d_backward,
        ) = D_Loss_BackProp(
            generator, discriminator, real_img, args, device, loss_dict, d_optim
        )
        ...
        (
            time_train_G_g_forward,
            time_train_G_d_forward,
            time_train_G_g_backward,
        ) = G_Loss_BackProp(
            generator, discriminator, args, device, loss_dict, g_optim, teacher_g
        )
        ...
        if iter_idx % 1 == 0:
            print(
                ...
                + " train_D_g_forward: "
                + str(round(time_train_D_g_forward, 3))
                + " train_D_d_forward: "
                + str(round(time_train_D_d_forward, 3))
                + " train_D_d_backward: "
                + str(round(time_train_D_d_backward, 3))
                + " D_Reg: "
                + str(round(reg_D_time, 3))
                + " train_G_g_forward: "
                + str(round(time_train_G_g_forward, 3))
                + " train_G_d_forward: "
                + str(round(time_train_G_d_forward, 3))
                + " train_G_g_backward: "
                + str(round(time_train_G_g_backward, 3))
                + " G_Reg: "
                + str(round(reg_G_time, 3))
            )
