...
parser.add_argument(
    "--channel_multiplier", type=int, default=train_hyperparams.channel_multiplier
)
...
parser.add_argument(
    "--load_train_state", type=bool, default=train_hyperparams.load_train_state
)
...
parser.add_argument(
    "--path_regularize", type=float, default=train_hyperparams.generator_path_reg_weight
)
parser.add_argument(
    "--path_batch_shrink", type=int, default=train_hyperparams.path_reg_batch_shrink
)
...
parser.add_argument(
    "--val_sample_freq", type=int, default=train_hyperparams.val_sample_freq
)
parser.add_argument(
    "--model_save_freq", type=int, default=train_hyperparams.model_save_freq
)
...
parser.add_argument(
    "--kd_l1_lambda", type=float, default=train_hyperparams.kd_l1_lambda
)
parser.add_argument(
    "--kd_lpips_lambda", type=float, default=train_hyperparams.kd_lpips_lambda
)
...
def Print_Experiment_Status(exp_log_file):
    ...
    experiment_status_str += (
        ...
        + "\n"
        + "    GPU Device IDs: "
        + str(train_hyperparams.gpu_device_ids)
        + "\n"
        + "    Number of GPUs: "
        + str(n_gpu)
        + "\n\n"
    ...
    if args.teacher_ckpt is not None:
        experiment_status_str += (
            ...
            + str(args.kd_mode)
            + "\n"
            + "    LPIPS Knowledge Distillation Weight: "
            + str(args.kd_lpips_lambda)
            + "\n"
            + "    Content Aware: "
            + str(args.content_aware_KD)
            + "\n\n"
        )
    else:
...
def Downsample_Image_256(im_tensor):
    im_tensor = F.interpolate(
        im_tensor, size=(256, 256), mode="bilinear", align_corners=False
    )
    return im_tensor
...
def KD_loss(
    ...
    fake_img_teacher_list = teacher_g(
    ...
    if parsing_net is not None:
        ...
        fake_img_teacher = Get_Masked_Tensor(
            fake_img_teacher, teacher_img_parsing, device, mask_grad=False
        )
        fake_img = Get_Masked_Tensor(
            fake_img, teacher_img_parsing, device, mask_grad=True
        )
    ...
    if percept_loss is None:
        ...
        if (
            args.size > train_hyperparams.LPIPS_IMAGE_SIZE
            ...
            kd_lpips_loss = args.kd_lpips_lambda * torch.mean(
                percept_loss(pooled_fake_img, pooled_fake_img_teacher)
            )
...
def train(
    ...
    for iter_idx in tqdm(
        range(args.start_iter, args.iter), ncols=70, initial=args.start_iter
    ):
        time1 = time.time()
        ...
        if (iter_idx + 1) % 1 == 0:
            ...
            logger.add_scalar("D_Loss", d_loss_val, iter_idx)
            logger.add_scalar("G_Loss", g_loss_val, iter_idx)
            logger.add_scalar("KD_L1_Loss", kd_l1_loss_val, iter_idx)
            logger.add_scalar("KD_LPIPS_Loss", kd_lpips_loss_val, iter_idx)
            logger.add_scalar("D_Reg", r1_val, iter_idx)
            logger.add_scalar("G_Reg", path_loss_val, iter_idx)
            logger.add_scalar("G_Mean_Path", mean_path_length_avg, iter_idx)
        ...
        if ((iter_idx + 1) % args.model_save_freq == 0) and (iter_idx > 0):
            ...
            g_ema_fid = Get_Model_FID_Score(
                ...
                device=device,
                gpu_device_ids=train_hyperparams.gpu_device_ids,
                info_print=False,
            )

            exp_log_file.write("\n" + "Evaluated FID: " + str(g_ema_fid) + "\n\n")
            logger.add_scalar("FID", g_ema_fid, iter_idx)
...
if __name__ == "__main__":
    ...
    if args.distributed:
        generator = nn.DataParallel(
            generator, device_ids=train_hyperparams.gpu_device_ids
        )
        discriminator = nn.DataParallel(
            discriminator, device_ids=train_hyperparams.gpu_device_ids
        )
        if teacher_g is not None:
            teacher_g = nn.DataParallel(
                teacher_g, device_ids=train_hyperparams.gpu_device_ids
            )
