...
def Generate_Interpolated_Image(
    generator, batch_size, eps, device, latent_dim, gpu_device_ids=None
):
    """
    Usage:
    ...
    with torch.no_grad():
        ...
        image = generator(
            noise_z=None, latent_styles=[latent_e], input_is_latent=True, noise=None
        )
...
def Get_PPL_Score(
    ...
    with torch.no_grad():
        ...
        percept = lpips.PerceptualLoss(
            model="net-lin", net="vgg", use_gpu=True, gpu_ids=[gpu_device_ids[0]]
        )
        ...
        for idx, batch in enumerate(batch_sizes):
            ...
            image = Generate_Interpolated_Image(
                generator,
